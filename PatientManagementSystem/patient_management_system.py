...
def add_patient():
    def save_new_patient():
        ...
        save_data(patients)
        messagebox.showinfo(
            "Success", f"Patient {entry_name.get()} added successfully!")
        add_window.destroy()
...
def view_patients():
    ...
    for col_num, header in enumerate(headers):
        tk.Label(view_window, text=header, font=("Arial", 10, "bold")).grid(
            row=0, column=col_num, padx=10, pady=5)

    # Display patient details in the window
    row_num = 1
    for pid, details in patients.items():
        tk.Label(view_window, text=pid).grid(
            row=row_num, column=0, padx=10, pady=5)
        tk.Label(view_window, text=details["Name"]).grid(
            row=row_num, column=1, padx=10, pady=5)
        tk.Label(view_window, text=details["Age"]).grid(
            row=row_num, column=2, padx=10, pady=5)
        tk.Label(view_window, text=details["Gender"]).grid(
            row=row_num, column=3, padx=10, pady=5)
        tk.Label(view_window, text=details["Contact"]).grid(
            row=row_num, column=4, padx=10, pady=5)
        tk.Label(view_window, text=details["Address"]).grid(
            row=row_num, column=5, padx=10, pady=5)
        row_num += 1
